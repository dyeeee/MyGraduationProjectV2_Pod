    ...
    public var body: some View {
        ZStack {
            if(self.showFull && self.showBackground){
                self.closedPath
                    .offset(x: 0, y: 2)
                    .fill(LinearGradient(gradient: Gradient(colors: [.red, .orange, .white]), startPoint: .bottom, endPoint: .top))
            ...
            self.path
                ...
                .rotationEffect(.degrees(180), anchor: .center)
                .rotation3DEffect(.degrees(180), axis: (x: 0, y: 1, z: 0))
                .animation(Animation.easeOut(duration: 2).delay(1.5))
                .onAppear {
                    self.showFull = true
            }
            .onDisappear {
...
struct Line_Previews: PreviewProvider {
    static var previews: some View {
        GeometryReader{ geometry in
            Line(data: ChartData(points: [0,5,10,54]), frame: .constant(geometry.frame(in: .local)), touchLocation: .constant(CGPoint(x: 100, y: 12)), showIndicator: .constant(true), minDataValue: .constant(nil), maxDataValue: .constant(nil))
