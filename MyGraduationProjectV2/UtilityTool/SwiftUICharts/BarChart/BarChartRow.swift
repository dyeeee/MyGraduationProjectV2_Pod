...
public struct BarChartRow : View {
    ...
    public var body: some View {
        GeometryReader { geometry in
            if(self.showLegend){
            Legend(data: ChartData(points: self.data), frame: .constant(geometry.frame(in: .local)), hideHorizontalLines: .constant(false))
            }
                ...
                ForEach(0..<self.data.count, id: \.self) { i in
                    VStack {
                        BarChartCell(value: self.normalizedValue(index: i),
                                     index: i,
                                     width: Float(geometry.frame(in: .local).width - 25),
